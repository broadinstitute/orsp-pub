...
class IrbTable extends Component {
  ...
  //   if (nextProps.applyFilter && nextProps.current !== prevState.current){
  ...
  tableHandler = async (offset, limit, search, sort, page) => {
    ...
    let filter = {
      before: this.props.beforeDate && this.props.applyFilter ? this.props.beforeDate.toLocaleDateString('en-US') : null,
      after: this.props.afterDate && this.props.applyFilter ? this.props.afterDate.toLocaleDateString('en-US') : null,
      projectType: this.props.applyFilter ? this.props.projectType.value : null
    };

    console.log('filter from irb table => ', filter);

    spinnerService.show(QA_REPORT_SPINNER);
    let result = await Reports.getQaEventReport(query, 'IRB', filter);
  ...
  //   this.setState(prev => {
  //     prev.beforeDate = filter.beforeDate;
  //     prev.afterDate = filter.afterDate;
  //     prev.projectType = filter.projectType;
  //     return prev;
  //   }, async () => await this.tableHandler(0, this.state.sizePerPage, this.state.search, this.state.sort, this.state.currentPage));
  // };
