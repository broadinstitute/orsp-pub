...
class SchedulerService {
    ...
    def generateUnApprovedCohortReport() {
        List csvData = [['Consent Key', 'Title', 'Information Sub-Status', 'Document Sub-Status', 'Project Key', 'Project Status']]
        SessionFactory sessionFactory = grailsApplication.getMainContext().getBean('sessionFactory')
        final session = sessionFactory.currentSession

        final String query = new StringBuilder()
                .append('SELECT * FROM (')
                .append('SELECT consent_filter.consent_key as "Consent Key", ')
                .append('CONCAT(\'"\', REPLACE(REPLACE(REPLACE(i2.summary, \'"\', \'""\'), \'\\r\', \' \'), \'\\n\', \' \'), \'"\') AS Title, ')
                .append('CASE ')
                .append('WHEN LOWER(isx.value) IN (\'1\', \'true\') THEN \'Approved\' ')
                .append('ELSE \'Pending\' ')
                .append('END AS "Information Sub-Status", ')
                .append('CASE ')
                .append('WHEN SUM(CASE WHEN LOWER(sd.status) = \'pending\' THEN 1 ELSE 0 END) > 0 THEN \'Pending\' ')
                .append('ELSE \'Approved\' ')
                .append('END AS "Document Sub-Status", ')
                .append('consent_filter.project_key as "Project Key", ')
                .append('i2.approval_status as "Project Status" ')
                .append('FROM ( ')
                .append('SELECT ')
                .append('ccl.consent_key, ')
                .append('ccl.project_key, ')
                .append('i.summary AS Title ')
                .append('FROM consent_collection_link ccl ')
                .append('LEFT JOIN issue i ')
                .append('ON ccl.consent_key = i.project_key ')
                .append('WHERE i.deleted = 0 ')
                .append(') AS consent_filter ')
                .append('LEFT JOIN issue i2 ')
                .append('ON consent_filter.project_key = i2.project_key ')
                .append('LEFT JOIN issue_extra_property isx ')
                .append('ON consent_filter.consent_key = isx.project_key AND isx.name = \'projectReviewApproved\' AND isx.deleted = 0 ')
                .append('LEFT JOIN storage_document sd ')
                .append('ON sd.project_key = consent_filter.consent_key AND sd.deleted = 0 ')
                .append('WHERE i2.deleted = 0 AND i2.approval_status NOT IN (\'withdrawn\', \'disapproved\', \'closed\', \'abandoned\', \'Legacy\') ')
                .append('GROUP BY ')
                .append('consent_filter.consent_key, ')
                .append('consent_filter.project_key, ')
                .append('i2.summary, ')
                .append('i2.approval_status, ')
                .append('isx.value ')
                .append(') as status_filtered WHERE ')
                .append('NOT ( ')
                .append('`Information Sub-Status` = \'Approved\' ')
                .append('AND `Document Sub-Status` = \'Approved\' ')
                .append(');')
                .toString()
