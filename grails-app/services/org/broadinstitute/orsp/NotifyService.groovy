...
class NotifyService implements SendgridSupport, Status {
    ...
    Map<Boolean, String> sendApprovedNotification(Issue issue, String author) {
        ...
        NotifyArguments arguments = new NotifyArguments(
                ...
                fromAddress: getDefaultFromAddress(),
                ccAddresses: [],
                subject: issue.projectKey + " - Your ORSP submission has been approved by " + author,
                issue: issue,
    ...
    Map<Boolean, String> sendRejectionProjectNotification(Issue issue, String user) {
        Collection<User> usersToNotify = userService.findUsers(issue.getPMs())
        Collection<String> emails = usersToNotify.emailAddress
        NotifyArguments arguments = new NotifyArguments(
                toAddresses: emails,
                fromAddress: getDefaultFromAddress(),
                ccAddresses: [],
                subject: issue.projectKey + " - Your ORSP submission has been disapproved by " + user,
                issue: issue,
                user:  userService.findUser(issue.reporter)
        )
    ...
    def sendProjectStatusNotification(String type, Issue issue, String user) {
        if (type?.equals(APPROVED)) {
            sendApprovedNotification(issue, user)
        } else if (type?.equals(DISAPPROVED)) {
            sendRejectionProjectNotification(issue, user)
    ...
    Map<Boolean, String> sendEditsApprovedNotification(Issue issue, String editCreatorName, String subjectDisplayUsername) {
        ...
        NotifyArguments arguments =
                new NotifyArguments(
                        ...
                        fromAddress: getDefaultFromAddress(),
                        subject: issue.projectKey + " - Your edits to this ORSP " + type + " have been approved by " + subjectDisplayUsername,
                        user: user,
                        issue: issue)
    ...
    Map<Boolean, String> sendEditsDisapprovedNotification(Issue issue, String editCreatorName, String displayUsername) {
        String type = issue.type?.equals(IssueType.CONSENT_GROUP.getName()) ? "Consent Group" : "Project"
        User user = userService.findUser(issue.reporter)
        NotifyArguments arguments =
                new NotifyArguments(
                        ...
                        ccAddresses: editCreatorName != null ? Collections.singletonList(userService.findUser(editCreatorName).emailAddress) : Collections.emptyList(),
                        fromAddress: getDefaultFromAddress(),
                        subject: issue.projectKey + " - Your edits to this ORSP " + type + " have been disapproved by " + displayUsername,
                        user: user,
                        issue: issue)
    ...
    Map<Boolean, String> sendApproveRejectLinkNotification(String projectKey, String consentKey, boolean isApproved, String subjectDisplayName) {
        Map<String, String> values = new HashMap<>()
