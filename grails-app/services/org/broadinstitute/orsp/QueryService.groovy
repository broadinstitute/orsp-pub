 ...
 */
    ...
    List getIRBReportData() {
        ...
        final String query = new StringBuilder()
                                    .append("SELECT t1.project_key, t1.type, t1.status, t1.approval_status, t1.request_date, ")
                                    .append("JSON_OBJECTAGG(t2.name, t2.value) as extra_properties, t3.funding_source, t3.funding_name ")
                                    .append("FROM orsp_dev.issue t1 ")
                                    .append("INNER JOIN orsp_dev.issue_extra_property t2 ON t1.project_key = t2.project_key ")
                                    .append("INNER JOIN (SELECT project_key, source as 'funding_source', name as 'funding_name' FROM funding) t3 ON t1.project_key = t3.project_key ")
                                    .append("WHERE t1.type = 'IRB Project' AND (t1.status = 'Approved' OR t1.approval_status = 'Approved') ")
                                    .append("GROUP BY t1.project_key, t1.type, t1.status, t1.approval_status, t1.request_date, t3.funding_source, t3.funding_name")
        final SQLQuery sqlQuery = session.createSQLQuery(query)
        final result = sqlQuery.with {
            list()
        }
