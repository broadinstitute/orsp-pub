...
//            case 5: Not easily sortable or filterable
    ...
    def findAllDataLocationsBySampleCollectionId(String consentCollectionId) {
        final session = sessionFactory.currentSession
        final String query =
                'select * from data_locations where collection_link_id = :consentCollectionIds and deleted = 0'
        final SQLQuery sqlQuery = session.createSQLQuery(query)
        final results = sqlQuery.with {
            addEntity(DataLocations)
            setString('consentCollectionIds', consentCollectionId)
            list()
        }
        results
    }
    ...
    List<ConsentCollectionLinkDTO> getCollectionLinksDtoByConsentKey(String consentKey) {
        ...
        ' select c.id id, c.consent_key consentKey, c.project_key linkedProjectKey, c.pii pii, c.compliance compliance, c.sharing_type sharingType , c.text_sharing_type textSharingType,  ' +
                ...
                ' inner join issue ip on ip.project_key = c.project_key ' +
                ' left join sample_collection sc on sc.collection_id = c.sample_collection_id' +
                ' where c.consent_key = :consentKey and c.deleted = 0 and c.is_latest="Y"'
 ...
 */
    ...
    Collection<Organization> getOrganizations() {
        SessionFactory sessionFactory = grailsApplication.getMainContext().getBean('sessionFactory')
        final session = sessionFactory.currentSession
        final String query = ' select o.*  from organization as o where o.deleted = 0 and o.active = 1 order by o.name asc'
        final SQLQuery sqlQuery = session.createSQLQuery(query)
    ...
    Collection<LogintTextResponse> getLoginTextResponse() {
        SessionFactory sessionFactory = grailsApplication.getMainContext().getBean('sessionFactory')
        final session = sessionFactory.currentSession
        final String query = ' select o.* from login_text_response o '
        final SQLQuery sqlQuery = session.createSQLQuery(query)
        final result = sqlQuery.with {
            list()
        }
    ...
    List getIRBReportData() {
        SessionFactory sessionFactory = grailsApplication.getMainContext().getBean('sessionFactory')
        final session = sessionFactory.currentSession
        def query = new StringBuilder()
        query.append("SELECT t1.project_key, JSON_OBJECTAGG(t2.name, t2_values_aggregated) as extra_properties, t3.funding_source ")
        query.append("FROM issue t1 ")
        query.append("INNER JOIN issue_extra_property t2 ON t1.project_key = t2.project_key ")
        query.append("LEFT JOIN ( ")
        query.append("SELECT project_key, CONCAT(GROUP_CONCAT(CONCAT(' ', source, ': \"', name, '\"'))) as funding_source ")
        query.append("FROM funding GROUP BY project_key ) t3 ON t1.project_key = t3.project_key ")
        query.append("LEFT JOIN ( SELECT project_key, name, GROUP_CONCAT(value) as t2_values_aggregated ")
        query.append("FROM issue_extra_property GROUP BY project_key, name ) t2_aggregated ON t1.project_key = t2_aggregated.project_key AND t2.name = t2_aggregated.name ")
        query.append("WHERE t1.type = 'IRB Project' AND ( ")
        query.append("(t1.status = 'Approved' AND t1.approval_status != \"Closed\" AND t1.approval_status != \"Withdrawn\" AND t1.approval_status != \"Abandoned\") ")
        query.append("OR t1.approval_status = 'Approved') ")
        query.append("GROUP BY t1.project_key, t1.request_date ")
        query.append("ORDER BY t1.request_date;")
    ...
    def getStorageDocUuid(String id, String type) {
        SessionFactory sessionFactory = grailsApplication.getMainContext().getBean('sessionFactory')
        final session = sessionFactory.currentSession
        String query = 'select id from storage_document where consent_collection_link_id= :id and file_type= :type'
        final SQLQuery sqlQuery = session.createSQLQuery(query)
        sqlQuery.setParameter('id', id)
    ...
    def findProjectKeyIfChanged(StorageDocument document) {
        ...
        // Below is the date when ORSP prefix change script was run on production
        // to change older legacy projects ORSP-XXXX prefix to the corresponding project prefix
